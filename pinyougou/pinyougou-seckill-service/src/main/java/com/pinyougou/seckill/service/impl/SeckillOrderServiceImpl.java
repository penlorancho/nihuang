...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long seckillId, String userId) {
		...
		//2.扣减redis库存(只有当缓存中的商品数减为0后，才将商品同步到数据库)
		...
		//3.存储秒杀订单（不向数据库存，只向缓存中存，支付成功后才向数据库中存）
		TbSeckillOrder seckillOrder = new TbSeckillOrder();
		seckillOrder.setId(idWorker.nextId());//账单流水号
		seckillOrder.setSeckillId(seckillId);//秒杀商品号
		...
		System.out.println("保存订单到redis成功");
	...
	@Override
	public void deleteOrderRromRedis(String userId, Long orderId) {
		//1.查询出缓存中的订单
